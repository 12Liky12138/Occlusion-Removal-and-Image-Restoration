...
class MaskWeightedCrossEntropyLoss(nn.Module):
    ...
    def __init__(self, inmask_weight=5, outmask_weight=1):
        ...
        # 自适应权重
        self.inmask_weight = nn.Parameter(torch.tensor(5.), requires_grad=True)
        self.outmask_weight = nn.Parameter(torch.tensor(1.), requires_grad=True)
    ...
    def forward(self, predict, target, mask):
        ...
        canshu = (~mask)/mask
        inweight = canshu*(loss_outmask/loss_inmask)
        outweight =1.0
        print(loss_outmask/loss_inmask)
        print(inweight)
        print(outweight)
        loss = (inweight*loss_inmask + outweight*loss_outmask)/ (n * h * w)
