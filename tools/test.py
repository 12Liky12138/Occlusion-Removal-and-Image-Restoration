...
class Tester(object):
    ...
    def infer(self):
        ...
        # # 参考c2f-segm
        # e = 1e-6
        # iou = intersection_rec.sum/(union_rec.sum+e)
        # print(iou)
        #
        # smooth = 1e-10
        # # intersection = ((amodal_pred == 1) & (amodal_gt == 1)).sum()
        # # intersection = (amodal_pred & amodal_gt).sum((-1,-2)).to(torch.float32)
        # # union = (amodal_pred | amodal_gt).sum((-1,-2)).to(torch.float32)
        # iou2 = (intersection_rec.sum+smooth)/(union_rec.sum+smooth)
        # print(iou2)

        print("Evaluation results. acc_allpair: {:.5g}, acc_occpair: {:.5g} \
                      mIoU: {:.5g}, pAcc: {:.5g}, inv_mIoU: {:.5g}".format(acc_allpair, acc_occpair, miou, pacc,
                                                                           inv_miou))
    ...
    # def make_COCOA_output(self, idx, amodal_pred, category):
    #     results = []
    #     for i in range(amodal_pred.shape[0]):
    #         data = dict()
    #         rle = maskUtils.encode(
    #             np.array(amodal_pred[i, :, :, np.newaxis], order='F'))[0]
    #         if hasattr(self.data_reader, 'indexing'):
    #             data['image_id'] = self.data_reader.indexing[idx]
    #         data['category_id'] = category[i].item()
    #         if isinstance(rle['counts'], bytes):
    #             rle['counts'] = rle['counts'].decode()
    #         data['segmentation'] = rle
